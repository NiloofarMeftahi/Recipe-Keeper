...
class UserItem(Resource):
    ...
    def get(self, user):
        ...
        #body.add_control("self", url_for("single_user", user=fetched_user))
        #base_uri = api.url_for(UserItem, user=fetched_user)
        body.add_control("self", api.url_for(UserItem, user=fetched_user))
        body.add_control_user(user=user)
        ...
        if user is None:
            body.add_control_get_recipe()
        else:
            body.add_control_get_recipe(user=user)
            print("User", fetched_user)
...
class UsersCollection(Resource):
    ...
    def get(self):
        ...
        for user in Users.query.all():
            ...
            item.add_control("self", api.url_for(UserItem, user=user.username))
