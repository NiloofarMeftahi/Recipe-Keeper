...
class RecipeBuilder(MasonBuilder):
    ...
    def add_control_get_recipe(self,user):
        ...
        if user is None:
            base_url=url_for("recipes")
        else:
            base_url=url_for("userrecipes", user=user)
    ...
    def add_control_delete_recipe(self,recipe,user):
        self.add_control_delete(
            "Delete the recipe",
            api.url_for(resources.recipe.RecipeItem, recipe=recipe ,user= user)
        )
    def add_control_edit_recipe(self,recipe,user):
        self.add_control_put(
            "Edit the Recipe",
            api.url_for(resources.recipe.RecipeItem, recipe=recipe ,user= user),
            Recipes.json_schema()
    ...
    def add_control_get_comments(self, user , recipe):
        self.add_control(
            "recipe:comments",
            api.url_for(resources.comment.CommentsCollection, user=user, recipe=recipe),
            isHrefTemplate=True,
        )
    def add_control_add_comments(self, user , recipe):
        print("bfbfb")
        self.add_control_post(
            "recipe:add_comment_to_this_recipe",
            "Add a new comment",
            api.url_for(resources.comment.CommentsCollection, user=user, recipe=recipe),
            Comment.json_schema()
        )
    def add_control_delete_comment(self,recipe,user):
        self.add_control_delete(
            "Delete the comment",
            api.url_for(resources.comment.CommentItem, recipe=recipe ,user= user)
        )
    def add_control_edit_comment(self,recipe,user):
        self.add_control_put(
            "Edit the Comment",
            api.url_for(resources.comment.CommentItem, recipe=recipe ,user= user),
            Comment.json_schema()
